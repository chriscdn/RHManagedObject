...
Before generating your classes you must ensure the `Class` setting on your entities are set to the entity names.  That is, open the xcdatamodeld in XCode, select the entity, and set the Class property to the right to the entity name.  In the employee example the class would be set to `EmployeeEntity`.

After your entity classes have been generated you must go back into the xcdatamodeld and change the `Class` property to your entity subclass.  In the employee example this would be `Employee`.  Also, the generated class must be modified to inherit from `RHManagedObjectClass` instead of `NSManagedObjectClass`. It's a small hack, but only means changing two lines of code. 
