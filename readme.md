...
(This hierarchy is slight deviation from previous versions of RHManagedObject.  See the section on [upgrading](#upgrading) for more information.)

The `RHManagedObject` base class extends the capabilities of NSManagedObject to make Core Data easier to use.  Specifically, it:
...
Notice that none of the examples require use of an `NSManagedObjectContext` instance. That's handled for you within the library. Of course, a method is available to fetch the object context for the current thread if required:

	NSManagedObjectContext *moc = [Employee managedObjectContextForCurrentThreadWithError:&error];
	
The `_Employee` class is an artefact of mogenerator, which is discussed in [setup](#setup).

## Installation

### Cocoapods

If you're using [Cocoapods](http://cocoapods.org/) you can include RHManagedObject in your Podfile as follows:
...
- copy `RHManagedObject.h`, `RHManagedObject.m`, `RHManagedObjectContextManager.h`, and `RHManagedObjectContextManager.m` into your project; and
- include the CoreData framework in your project.

## Setup

The easiest way to begin using RHManagedObject is to use [mogenerator](https://github.com/rentzsch/mogenerator).  Mogenerator automates the generation of the model classes, but also provides a place for custom logic that won't be overwritten when the classes are regenerated.
...
For example, our `Employee` model has the following hierarchy:

	NSObject :: NSManagedObject :: RHManagedObject :: _Employee :: Employee

The `_Employee` class is generated by mogenerator and is overwritten when the model is changed and regenerated.  For this reason it should never be manually edited.  The `Employee` class is generated once (unless the file already exits) and provides a place where additional methods can be added without disrupting the generated entity class.  The one requirement is a method to identify to which model it belongs. This method is used by the `RHManagedObject` superclass and looks like the following in the `Employee` example:
...
However, it's also the place where additional methods can be added. For example:
...
This is transparent to the rest of your application, but requires some changes to how your Core Data model is setup.

This requires a few small changes:

- You'll need to perform a lightweight migration of your model (see [Lightweight Migration](#lightweight-migration) for details).
- The `Class` property in the `xcdatamodeld` should be the same as your `Name` field (in this case "Employee").
- "Renaming ID" 
- Drop `-entityName`.
