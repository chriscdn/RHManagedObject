...
This should be mostly transparent to the rest of your application, but requires some changes in the model setup.

This requires a few small changes:

- You'll need to perform a lightweight migration of your model (see [Lightweight Migration](#lightweight-migration) for details).  Most of this involves the renaming of your entities (e.g., `EmployeeEntity` to `Employee` in the example).  For this you can use the "Renaming ID" field in XCode, which tells the migration how your models get renamed. 
- As described in the [mogenerator documentation](http://raptureinvenice.com/getting-started-with-mogenerator/), the `Class` property in the `xcdatamodeld` should be the same as your `Name` field (in the example "Employee").
