...
- Core Data is not thread safe. If you wish to mutate your objects off the main thread you need to create a managed object context in that thread, attach a `NSManagedObjectContextDidSaveNotification` notification to it, and merge the context into the main object context in an observer method on the main thread.  Bleh.  RHManagedObject does this for you so that you can work with your objects in different threads without having to think about this.
...
- Managing multiple models becomes tricky with the standard Core Data design pattern.  RHManagedObject supports multiple models to make this simple and transparent.

## Overview

This brief overview assumes you have some experience with Core Data.

A typical Core Data "Employee" entity (say, with attributes `firstName` and `lastName`) generated by [mogenerator](https://github.com/rentzsch/mogenerator) has an inheritance hierarchy of:
...
The `_Employee` class is generated by mogenator and will be discussed later, while the `RHManagedObject` class extends the capabilities of NSManagedObject to make Core Data easier to use.  Specifically, it:
...
The easiest way to begin using RHManagedObject is to use [mogenerator](https://github.com/rentzsch/mogenerator).  Mogenerator automates the generation of the model classes (which saves a lot of time), but also provides a place for custom logic that won't be overwritten if the classes are ever regenerated.

Once mogenator is setup you need to add `--base-class RHManagedObject` to the command, which ensures RHManagedObject is the superclass of the generated models. This could look as follows:
...
For example, with our `Employee` model the hierarchy would be as follows:

	NSObject :: NSManagedObject :: RHManagedObject :: _Employee :: Employee

The `_Employee` class is generated by mogenerator and is overwritten if the model is ever changed and regenerated.  It should never be edited.  The `Employee` class provides a place to add custom logic, but also requires a method to identify to which model it belongs. This method is used by the `RHManagedObject` superclass and looks like the following in the `Employee` example:
